...
class RestoreNetworkConfiguration(ScopeExit):
    def __init__(self, camera):
        ...
        super().__init__(lambda: camera.apply_network_configuration(network_configuration))


def test_default_network_configuration():
    network_configuration = zivid.NetworkConfiguration()
    assert network_configuration.ipv4.mode == zivid.NetworkConfiguration.IPV4.Mode.manual
...
def test_fetch_network_configuration_while_connected(physical_camera):
    ...
    assert network_configuration.ipv4.mode in zivid.NetworkConfiguration.IPV4.Mode.valid_values()

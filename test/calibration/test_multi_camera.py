...
def test_multicamera_invalid_input_detectionresult_invalid(point_cloud):
    import pytest
    import zivid.calibration

    detection_result = zivid.calibration.detect_feature_points(point_cloud)
    detection_results = [detection_result, detection_result]

    with pytest.raises(RuntimeError):
        # Will fail because detection results are not valid
        zivid.calibration.calibrate_multi_camera(detection_results)


def test_multicamera_invalid_input_single_detectionresult(point_cloud):
    import pytest
    import zivid.calibration

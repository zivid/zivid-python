"""Module containing implementation of infield correction functionality.
...
class InfieldCorrectionDetectionStatus:
    """Container for the status of a single detection result."""
    ...
    _valid_values = {
        "ok": _zivid.infield_correction.InfieldCorrectionDetectionStatus.ok,
        "detectionFailed": _zivid.infield_correction.InfieldCorrectionDetectionStatus.detection_failed,
        "invalidAlignment": _zivid.infield_correction.InfieldCorrectionDetectionStatus.invalid_alignment,
    }
...
class InfieldCorrectionInput:
    ...
    def status(self):
        ...
        """
        for key, internal_value in InfieldCorrectionDetectionStatus.valid_values().items():
            if internal_value.value == self.__impl.status().value:
                return key
        raise ValueError("Unsupported value {value}".format(value=self.__impl.status))
